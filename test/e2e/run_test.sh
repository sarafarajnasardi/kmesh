...
function setup_kmesh() {
	...
	# Compare versions: if <= 1.26, use legacy format
	if awk -v ver="$istio_version_major_minor" 'BEGIN { exit !(ver <= 1.26) }'; then
		echo "Detected Istio version $ISTIO_VERSION (<= 1.26), enabling legacy EnvoyFilter format"
		extra_args="$extra_args --set envoyFilter.useLegacyFormat=true"
	else
